...
Before an isolated Schedule Builder scan, V3 stays on the current authenticated page and clears pre-existing/enrolled course cards individually using the page's trash controls. It does not navigate to a blanket clear/reset URL. Course selection then requires an exact canonical course-code match, and parsed result components are rejected if their course code does not exactly match the request.
