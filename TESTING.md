...
- Start VSB with five enrolled courses, then repeat with seven. Confirm every newly created worker clears the exact number present and does not print `READY` until its active-course snapshot is zero; no cleanup path may assume a five-course load.
- Create a schedule where one professor has zero compatible combinations while another remains available (for example, 1 matching schedule out of 2 section choices). After analysis completes, confirm the unavailable professor card dims immediately and shows `Unavailable with current constraints` without requiring another unrelated UI refresh.
...
- Add a new course while deep verification is active. Confirm each deep worker finishes its current VSB result, logs `YIELDED ...; READY`, the new course receives its preliminary fast timetable first, and deep verification later resumes without discarding completed work. If PHYS 2401 paused at 28/60, the next deep pass must report a saved checkpoint and plan ranges over the remaining indexes rather than restart at result 1.
- Let an isolated worker sit idle long enough to expire (or force its readiness probe back to login). On its next lease, confirm it is logged `FAILED`, closed/removed, and a later capacity request creates a fresh monotonic worker ID/session.
- Force a worker startup timeout. Confirm the failed browser/context is closed and the pool attempts a brand-new isolated session instead of entering the old one-minute global parallel-disable state. A successful later session must report its own UUID marker/fingerprint and `READY`.
